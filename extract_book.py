...
def get_book_data(EXAMPLE_URL):
    ...
    if response.ok:  # si justement ok
        ...
        product_page_url = url  # soup.find pour nom classe exacte, soup.select nom classe inexaxte
        universal_product_code = soup.find('th', text='UPC').find_next_sibling(
            'td').text  # colonne suivante, conversion txt
        title = soup.select('div.col-sm-6.product_main > h1')[0].text  # unique indice, conversion txt
        price_excluding_tax = soup.find('th', text='Price (excl. tax)').find_next_sibling('td').text.replace('Â',
                                                                                                             '')  # comme title, enleve specialchar
        price_including_tax = soup.find('th', text='Price (incl. tax)').find_next_sibling('td').text.replace('Â',
                                                                                                             '')  # comme title, enleve specialchar
        number_available = soup.find('th', text='Availability').find_next_sibling(
            'td').text  # comme title, conversion txt
        product_description = soup.find('div', {'id': 'product_description'}).find_next_sibling(
            'p').text  # comme title, balises differentes
        category = soup.find('li', {'class': 'active'}).find_previous_sibling('li').find(
            'a').text  # inverse title, balises differentes, recuperation lien
